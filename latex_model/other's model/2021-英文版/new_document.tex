...
% 便于修改字体：fontspec需在ctexart后、其他字体宏包前加载
\usepackage{fontspec}
% 设置中文字体（Windows自带宋体，避免中文显示异常）
\setCJKmainfont{SimSun}
% 设置默认英文字体（Times New Roman，Windows自带）
\setmainfont{Times New Roman}
% 设置等宽的代码字体：替换IBM Plex Mono为Windows自带的Courier New
...
% 设置参考文献不输出默认名
\usepackage{etoolbox}
% 删除newtxtext：与fontspec冲突，且已用fontspec设置Times New Roman
...
% 第一页的字体为times new roman（已通过fontspec设置，无需重复设置）
\thispagestyle{empty}
...
% 开始写 memo 信
% 更换字体为 palatino：替换texgyrepagella-regular.otf为Windows自带的Palatino Linotype
